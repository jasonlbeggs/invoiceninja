<div>
    @if($errors->any())
        <div class="-mt-4 alert alert-failure mb-4">
    ...
    @if($mode == 'table')
        <div class="flex items-center gap-2">
            <button type="button" class="button button-primary bg-primary" wire:click.prevent="startDownload">{{ ctrans('texts.download') }}</button>
            @if($this->gateway_available)
                <button type="button" class="button button-primary bg-primary" wire:click.prevent="startPayment">{{ ctrans('texts.pay_now') }}</button>
            @endif
        </div>
        <div class="flex flex-col mt-4">
            <div>
                        ...
                        <table class="min-w-full mt-4 border border-gray-200 rounded shadow invoices-table">
                            <thead>
                                <tr>
                                    <th class="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-white uppercase border-b border-gray-200 bg-primary">
                                        <label>
                                            <input wire:model.live="all_selected" type="checkbox" class="form-check form-check-parent">
                                        </label>
                                    </th>
                                    <th class="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-white uppercase border-b border-gray-200 bg-primary">
                            ...
                            <tbody>
                                @forelse($this->invoices as $invoice)
                                    <tr wire:key="invoice-{{ $invoice->hashed_id }}" class="bg-white group hover:bg-gray-100">
                                        <td class="px-6 py-4 text-sm font-medium leading-5 text-gray-900 whitespace-nowrap">
                                            <label>
                                                <input wire:model.live="selected_invoice_ids" type="checkbox" class="form-check form-check-child" value="{{ $invoice->hashed_id }}">
                                            </label>
                                        </td>
                                        ...
                                        <td class="flex items-center justify-end px-6 py-4 text-sm font-medium leading-5 whitespace-nowrap">
                                            @if($invoice->isPayable() && $invoice->balance > 0 && $this->gateway_available)
                ...
                <div class="flex justify-center mt-6 mb-6 md:justify-between">
                    @if($this->invoices && $this->invoices->total() > 0)
                        <span class="hidden text-sm text-gray-700 md:block mr-2">
                            {{ ctrans('texts.showing_x_of', ['first' => $this->invoices->firstItem(), 'last' => $this->invoices->lastItem(), 'total' => $this->invoices->total()]) }}
                        </span>
                    @endif
                    {{ $this->invoices->links('portal/ninja2020/vendor/pagination') }}
                </div>
            </div>
        </div>
    @elseif ($mode == 'downloading')
        <div class="container mx-auto">
            <button wire:click.prevent="download" type="submit" class="button button-primary bg-primary">{{ ctrans('texts.download') }}</button>

            <div class="mt-4">
                @foreach($this->selected_invoices as $invoice)
                    <div wire:key="download-{{ $invoice->hashed_id }}">
                        <dl>
    ...
    @elseif ($mode == 'payment')
        TODO: refactor payment view here
    @endif
</div>
